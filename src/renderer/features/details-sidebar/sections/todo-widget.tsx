  ...
  return (
    <div className="mx-2 mb-2">
      ...
      <div
        ...
        aria-label={t("details.todosWidget.expandLabel", { count: totalTodos, action: isExpanded ? t("details.todosWidget.collapse") : t("details.todosWidget.expand") })}
        tabIndex={0}
        onKeyDown={handleKeyDown}
      >
        <div className="flex items-center gap-2 flex-1 min-w-0">
          <PlanIcon className="h-3.5 w-3.5 text-muted-foreground shrink-0" />
          <span className="text-xs font-medium text-foreground">{t("details.todosWidget.title")}</span>
          <span className="text-xs text-muted-foreground truncate flex-1">
            {todos[0]?.content || t("details.todosWidget.title")}
          </span>
