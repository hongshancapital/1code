          ...
          >
            ...
            >
              ...
              <div className="relative bg-muted rounded-md h-7 p-0.5 flex">
                <div
                  className="absolute inset-y-0.5 rounded bg-background shadow-xs transition-all duration-200 ease-in-out"
                  style={{
                    width: "calc(50% - 2px)",
        ...
        >
          ...
          <DiffSearchBar
            ...
            onScrollToFile={(fileIndex) => {
              // Force virtualizer to scroll to the file
              // First, use scrollToIndex which updates virtualizer's internal state
              virtualizer.scrollToIndex(fileIndex, { align: "start", behavior: "auto" })

              // Then use DOM scrolling as backup
              // The virtualizer element at this index should be rendered
              requestAnimationFrame(() => {
                // Try virtualizer again after frame
                virtualizer.scrollToIndex(fileIndex, { align: "start", behavior: "auto" })

                // Also try direct DOM scroll to the item
                const container = scrollContainerRef.current
                if (container) {
                  const items = virtualizer.getVirtualItems()
                  const targetItem = items.find(item => item.index === fileIndex)
                  if (targetItem) {
                    container.scrollTop = targetItem.start
                  }
                }
              })
            }}
