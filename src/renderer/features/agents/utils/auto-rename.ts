...
const autoRenameLog = createLogger("AutoRename")
...
export async function autoRenameAgentChat({
  ...
  autoRenameLog.info("Called with:", { subChatId, parentChatId, userMessage: userMessage.slice(0, 50), isFirstSubChat })
  ...
  try {
    ...
    autoRenameLog.info("Calling generateName (returns fallback, AI runs in background)...")
    const { name } = await generateName(userMessage)
    autoRenameLog.info("Got fallback name:", name)

    if (!name || name === "New Chat") {
      autoRenameLog.info("Skipping - generic name, confirming immediately")
      onNameConfirmed?.()
      return
    }
    ...
    for (let attempt = 0; attempt < delays.length; attempt++) {
      ...
      try {
        autoRenameLog.info(`Attempt ${attempt + 1}: setting fallback name "${name}"`)
        ...
        autoRenameLog.info(`✓ Fallback name set!`)
        break
      } catch (err) {
        autoRenameLog.warn(`Attempt ${attempt + 1} failed:`, (err as Error).message || err)
        if (attempt === delays.length - 1) {
          autoRenameLog.error(`Failed to set fallback name after ${delays.length} attempts`)
        }
      }
    }
    ...
    autoRenameLog.info("Fallback name set, waiting for name confirmation via IPC...")

    // Fallback: if IPC never arrives in 15s, confirm the name anyway
    // This handles edge cases where backend doesn't send IPC
    setTimeout(() => {
      autoRenameLog.info("Fallback timeout: confirming name after 15s")
      onNameConfirmed?.()
    }, 15_000)

  } catch (error) {
    autoRenameLog.error("Auto-rename failed:", error)
